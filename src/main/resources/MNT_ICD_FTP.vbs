'--------------------------------------------------------------------------- 
'-> Стартовая программа монитора связи с сервером FTP.ICD.RU *** 14.01.2021 *** 
'-- *** КОРНЕВАЯ ПАПКА ПРОГРАММЫ    -> C:\Apps\Sprut4\ICD_FTP     ( подсистема FTP-обмена с ICD )
'-- ***          ПАПКА со скриптами -> C:\Apps\Sprut4\ICD_FTP\PRG ( рабочие скрипты и копии рабочих программ )
'-- ***          ПАПКА с  архивами  -> C:\Apps\Sprut4\ICD_FTP\ARX ( архивная папка, вложения - папки IN, OUT, BAD, PROCESSED ) 
'-- ***          РАБОЧАЯ ПАПКА      -> C:\Apps\Sprut4\ICD_FTP\BOX ( буфер обмена,   вложения - папки IN, OUT, BAD, PROCESSED ) 
'-- ***          ЖУРНАЛЬНАЯ ПАПКА   -> C:\Apps\Sprut4\ICD_FTP\LOG ( журналы FTP-сессий и общий журнал обработки заявок в ГС  )
...
For Each objProcess in colProcesses   '<--- ЦИКЛ ПО СИСТЕМНЫМ ПРОЦЕССАМ !!! (анализ и защита от дублей процессов)
	...
	If objProcess.Name = "MNT_ICD_FTP.exe" and instr(objProcess.CommandLine, WScript.ScriptFullName) > 0 Then
		If wStr0 = "Y" Then
			rs = msgbox("ПАРАМЕТРЫ ПРОЦЕССА: " & chr(10) & chr(10) & " CommandLine -> " & objProcess.CommandLine & chr(10) & _
						...
						, vbYesNo,"   ВНИМАНИЕ !!!  FTP-МОНИТОР << ICD >> УЖЕ РАБОТАЕТ !!! ... " )
		...
		End If
	Else
		If objProcess.Name = "cmd.exe" and instr(objProcess.CommandLine, "ICD_FTP" ) > 0 Then
...
'-> VVV <-----------------------------------------------< ПРОВЕРКА условий для работы монитора
	wStr0 = "C:\Apps\Sprut4\ICD_FTP\PRG\ICD_SCP.scr" 
	If ( START_FLAG = "1" ) and ( not objFSO.FileExists(wStr0) ) Then '<- МОНИТОР ДОЛЖЕН ЗАВЕРШИТСЯ ?
		...
		msgbox chr(10) & chr(10) & "   ВНИМАНИЕ !!! " & _ 
				chr(10) & chr(10) & " FTP-МОНИТОР << ICD >> ЗАВЕРШАЕТ РАБОТУ " & _
...
'............................................................................-> Выборка пакета из БД
		Set OraDynaset = OST1.DbCreateDynaset( "SELECT * FROM KB_ICD_OUT WHERE processing_date is Null and status = 'N' " ,0)
...
'............................................................................-> Выгрузка XML-пакета в файл
			Set OraCLOB = OraDynaset.Fields("xml_data").Value
				...
				Ws0 = "C:\Apps\Sprut4\ICD_FTP\BOX\Out\" & OraDynaset.Fields("file_name").Value 
				OraCLOB.CopyToFile ws0
			Set OraCLOB = Nothing
			OraDynaset.MoveNext
		Wend
'............................................................................-> Отметка о выгрузке пакетов  	
		OST1.ExecuteSQL ( "UPDATE KB_ICD_OUT SET processing_date = sysdate, status = 'S' WHERE processing_date is Null and status = 'N' " )  
'............................................................................-> Закрытие транзакции 	
...
'                                            Открытие FTP-сессии
	   ...
	   fs.WriteLine "open sftp://severtrans_it:SevTraIt20XX@213.170.95.24 -hostkey=""*"""
...
'	   fs.WriteLine "lcd C:\Apps\Sprut4\ICD_FTP\BOX\In" 
'	   fs.WriteLine "get -delete *.xml"
'                                            Передача данных (уведомлений)
	   fs.WriteLine "cd /Outbound/Response"
	   fs.WriteLine "lcd C:\Apps\Sprut4\ICD_FTP\BOX\Out"
...
'->
	objShl.Run "C:\Apps\Sprut4\SVT_FTP_MNT_ALL\PRG\WinSCP.exe /console /script=" & wStr0 _ 
	         & " /log=c:\Apps\Sprut4\ICD_FTP\LOG\ICD_SCP_" & arxMrk0 & ".log", 0 , True 
	WScript.Sleep 10000 				' <-  даём время для "сна" mlsek
'->
'-> VVV <-----------------------------------------------< копируем переданные xml-файлы в архив
	objFSO.CopyFile "C:\Apps\Sprut4\ICD_FTP\BOX\Out\*.xml" , "C:\Apps\Sprut4\ICD_FTP\ARX\Out" , OverwriteExisting
	objFSO.CopyFile "C:\Apps\Sprut4\ICD_FTP\BOX\Bad\*.xml" , "C:\Apps\Sprut4\ICD_FTP\ARX\Bad" , OverwriteExisting
'-> VVV <-----------------------------------------------< удаляем из выходных буферов xml-файлы сохраненные в архиве
	objFSO.DeleteFile "C:\Apps\Sprut4\ICD_FTP\BOX\Out\*.xml",force
	objFSO.DeleteFile "C:\Apps\Sprut4\ICD_FTP\BOX\Bad\*.xml",force
	objFSO.DeleteFile "C:\Apps\Sprut4\ICD_FTP\BOX\processed\*.xml",force
'-> VVV <-----------------------------------------------< РАБОТА С АРХИВАМИ ( ЧИСТКА АРХИВОВ И ЖУРНАЛОВ )
	...
	if (( 200 < wi0 ) and ( wi0 < 225 )) then   '<- время архивации ( с 2,00 до 2,30 ) ? 
		...
		Set objFolder = objFSO.GetFolder("c:\Apps\Sprut4\ICD_FTP\LOG")
		...
		for wi0 = 0 to 2
			...
			Set objFolder = objFSO.GetFolder( "c:\Apps\Sprut4\ICD_FTP\ARX\" & ws2 )
			Set objFilelist = objFolder.files
			For Each objFile in objFilelist
				With objFile
...
'-> VVV <-----------------------------------------------< ОБРАБОТКА И ЗАГРУЗКА В ТАБЛИЦУ << KB_HKM_IN >> ПОЛУЧЕННЫХ С ftp-СЕРВЕРА ФАЙЛОВ (ПАКЕТОВ) :
	Set objFolder = objFSO.GetFolder("C:\Apps\Sprut4\ICD_FTP\BOX\In")
	Set objFilelist = objFolder.files
	...
	For Each objFile in objFilelist
		With objFile
			IF Ucase(Right(.name,4)) = ".XML" THEN  '<- Это посылка(ПАКЕТ) ? 
				IF .Size < 10 THEN                  '<- 1) бракуем "пустышки" ( Какова минимальная длина посылки ICD-FTP ? )
					.Move( "C:\Apps\Sprut4\ICD_FTP\BOX\Bad\" + .name ) 
				ELSE                                '<- 2) бракуем "пустышки" ( Какова минимальная длина посылки ICD-FTP ? )
					Set messages_id  = OST1.CreateDynaset( "select SEQ_KB_ICD_IN.NextVal as new_id FROM DUAL" , 0)
				'.........................................................................->  Получение нового ИД  
					new_id = messages_id.Fields("new_id").Value 
				'.........................................................................->  Вставка(добавление) новой строки в таблицу KB_HKM_IN 
					Ws2 = "C:\Apps\Sprut4\ICD_FTP\BOX\In\"
					Ws0 = "insert into KB_ICD_IN( message_id, message_data, message_name, message_delivery_date, message_status ) values( " _ 
					                              & new_id & " , empty_clob(), '" & Ws2 & .name & "', sysdate, 'N' )" 
	''' For TEST ->				msgbox " SQL 0 ->" & Ws0 
					OST1.ExecuteSQL ( Ws0 )  
					'.....................................................................->  Выбор добавленной в таблицу KB_HKM_IN записи	 
						Ws0 = "select * from KB_ICD_IN where message_id = " & new_id 
...
'-> VVV <-----------------------------------------------< копируем принятые xml-файлы в архив 
	objFSO.CopyFile "C:\Apps\Sprut4\ICD_FTP\BOX\In\*.xml" , "C:\Apps\Sprut4\ICD_FTP\ARX\In" , OverwriteExisting
'-> VVV <-----------------------------------------------< удаляем принятые xml-файлы в архив 
	objFSO.DeleteFile "C:\Apps\Sprut4\ICD_FTP\BOX\In\*.xml",force
'-> VVV <-----------------------------------------------< Чистка системных объектных переменных
	...
	Set objProcess = Nothing 
'->
'-> VVV <-----------------------------------------------< Ожидание начала следующего обащения к FTP.ICD_FTP.RU
'->
